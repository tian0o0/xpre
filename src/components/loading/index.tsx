...
function LoadingText(h: CreateElement, props: LoadingProps) {
  const { text, textSize, color } = props;
  if (text) {
    const fontSize = textSize && addUnit(textSize)
    const style = {
      fontSize,
      color
    }
...
function Loading(
  ...
  const { color, size, type, vertical } = props;
  ...
  return (
    <div class={bem([type, { vertical }])} {...inherit(ctx, true)}>
